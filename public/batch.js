...
function itemConditions(item, fallbackConditions) {
  if (Array.isArray(item.conditions) && item.conditions.length) {
    return item.conditions.map((condition) => String(condition).trim().toUpperCase()).filter(Boolean);
  }
...
async function runBatch() {
  ...
  for (const draft of drafts) {
    for (const condition of itemConditions(draft, conditions)) {
  ...
  for (const task of tasks) {
    ...
    try {
      const data = await requestFeedbackWithRetry({
        participantId,
...
async function requestFeedbackWithRetry(payload) {
  const maxAttempts = 3;
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      lastError = error;
      const shouldRetry =
        /429|too many requests|failed to fetch|timeout|network/i.test(String(error.message || error));

      if (!shouldRetry || attempt === maxAttempts) break;

      const waitMs = 10000 * attempt;
      logLine(`请求失败，${waitMs / 1000} 秒后重试第 ${attempt + 1} 次：${payload.participantId}`);
      await sleep(waitMs);
    }
  }
